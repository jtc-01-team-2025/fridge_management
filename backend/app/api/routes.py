...
@router.get("/items/", response_model=list[schemas.FridgeContents])
def read_items(db: Session = Depends(get_db)):
    return crud.get_items_sorted(db)


# まだ食べられるものの判定用endpoint
@router.get("/items/valid/", response_model=list[schemas.FridgeContents])
...
@router.get("/items/expired/", response_model=list[schemas.FridgeContents])
...
@router.get("/items/soon/", response_model=list[schemas.FridgeContents])
...
@router.get("/items/grouped/", response_model=dict[str, list[schemas.FridgeContents]])
