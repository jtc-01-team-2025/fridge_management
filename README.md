...
`backend/app/main.py` でデータベース初期化方法を選択できます：

### 【既存テーブルがある環境】マイグレーション機能付き（推奨）
```python
# カラム追加などのマイグレーションを自動実行
from app.db.init_db import init_db
```

### 【新規環境】シンプルなテーブル作成のみ
```python
# 新しいテーブルを作成するだけ（既存テーブルは変更しない）
from app.db.database import init_db
```

**切り替え方法**: `main.py` の該当行をコメントアウト/アンコメントするだけ
...
1. /frontend/の中に`.env` ファイルを作成して下記をペースト(＊初回のみ必須)
   API を取得する環境関数を作成しました。VITE_USE_API ない場合ダミーデータを取得します。
