...
const generateDummyItems = (): FoodType[] => {
  ...
  const dummyData: FoodType[] = [
    { id: 1, name: "牛乳", date_expiration: formatDate(new Date(today.getTime() + 1 * 24 * 60 * 60 * 1000)) }, // 明日
    { id: 2, name: "卵 (Lサイズ)", date_expiration: formatDate(new Date(today.getTime() + 7 * 24 * 60 * 60 * 1000)) }, // 7日後
    { id: 3, name: "鶏むね肉", date_expiration: formatDate(new Date(today.getTime())) }, // 今日が期限
    { id: 4, name: "キャベツ", date_expiration: formatDate(new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000)) }, // 30日後
    { id: 5, name: "納豆", date_expiration: formatDate(new Date(today.getTime() - 3 * 24 * 60 * 60 * 1000)) }, // 期限切れ
    { id: 6, name: "レタス", date_expiration: formatDate(new Date(today.getTime() + 10 * 24 * 60 * 60 * 1000)) },
  ];
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md transform transition-all duration-300">
        <div className="flex justify-between items-start border-b pb-3 mb-4">
          ...
          <button onClick={onClose} className="text-gray-500 hover:text-gray-900 text-2xl leading-none">
            &times;
          </button>
        </div>
        {children}
...
const ExpirationStatus = ({ date_expiration }: { date_expiration: string }) => {
  let label = "";
...
const ItemDeletePage = ({ items, onBack, onDeleteItem }: { 
  items: FoodType[]; 
  onBack: () => void; 
              ...
              > {/* 修正: '>'をonClick属性の行に結合 */}
                <div className="flex items-center space-x-4">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center border-2 ${
                      selectedItems.includes(item.id) ? 'bg-red-500 border-red-500' : 'bg-white border-gray-400'
                  }`}>
...
const ItemAddPage = ({ onBack, onAddItem }: { onBack: () => void; onAddItem: (name: string, days: number) => void }) => {
    ...
    return (
        <div className="p-6 md:p-10 bg-white rounded-xl shadow-2xl w-full max-w-md mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                    <label htmlFor="itemName" className="block text-sm font-medium text-gray-700">食材名</label>
                    <input
                        id="itemName"
                        type="text"
                        value={itemName}
                ...
                <div>
                    <label htmlFor="days" className="block text-sm font-medium text-gray-700">期限 (今日から何日後)</label>
                    <input
                        id="days"
                        type="number"
                        value={days}
...
const HomePage = ({ items, urgentItems, togglePopup, navigate, getStatusComponent }: {
  items: FoodType[];
  urgentItems: FoodType[];
  togglePopup: () => void;
  navigate: (path: 'home' | 'add' | 'delete') => void;
  getStatusComponent: (date: string) => JSX.Element;
}) => (
        ...
        {items.length === 0 ? (
          ...
          <ul className="space-y-3">
            {items.map((item) => (
              <li 
                key={item.id} 
                className="flex justify-between items-center p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition duration-200 border border-gray-100"> {/* 修正: '>'をclassName属性の行に結合 */}
                <div className="flex items-center space-x-4">
                ...
                {getStatusComponent(item.date_expiration)}
              </li>
            ))}
          </ul>
        )}
...
const Header = ({ navigate, currentPage }: { navigate: (path: Page) => void, currentPage: Page }) => (
...
export function App() {
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      default:
        return <HomePage items={data} urgentItems={urgentItems} togglePopup={togglePopup} navigate={navigate} getStatusComponent={ExpirationStatus} />;
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col p-0 font-sans">
      ...
      <PopUp isVisible={isPopupVisible} onClose={togglePopup}>
        <div className="space-y-4">
          {urgentItems.length === 0 ? (
            <p className="text-gray-600">期限が近い、または期限切れの食材はありません。素晴らしい！</p>
          ) : (
            <>
              ...
              <ul className="space-y-2 max-h-60 overflow-y-auto pr-2">
                {urgentItems
                  .map((item) => (
                    <li key={item.id} className="flex justify-between items-center p-3 bg-red-50 rounded-lg border border-red-200"> 
